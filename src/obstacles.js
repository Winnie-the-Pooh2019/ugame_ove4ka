...
export function spawnObstacle(state, IMGS, getAspect) {
  ...
  // Прогресс скорости 0..1 (для всех динамик)
  const prog = clamp(
  ...
  // Гарантия минимальной дистанции по «времени реакции»
  ...
  if (last) {
    ...
    // СТАРТОВЫЕ времена реакции из конфига
    const baseStartTime =
        last.kind === 'lowcloud'
            ? (SPACING.reactTimeSecCloud ?? 0.4)
            : (SPACING.reactTimeSecFence ?? 0.7);

    // Умягчаем рост дистанции к maxSpeed
    const kTo = clamp(SPACING.speedGapFactorTo ?? 0.8, 0.4, 1.0); // 1.0 = без изменений
    const timeEff = baseStartTime * (1 - prog + prog * kTo); // lerp(1 → kTo)

    const minGapPx =
        state.speed * timeEff +
        state.player.w * (SPACING.extraSheepWidths ?? 0.4);
  ...
  if (t < 0.65) {
    // Забор — размер управляется конфигом
    const usePx = Number.isFinite(FENCE_SIZE?.minPx) || Number.isFinite(FENCE_SIZE?.maxPx);

    let hMin = usePx
        ? (FENCE_SIZE.minPx ?? 12)
        : state.player.h * (FENCE_SIZE?.hMinFactor ?? 1.0);

    let hMax = usePx
        ? (FENCE_SIZE.maxPx ?? hMin)
        : state.player.h * (FENCE_SIZE?.hMaxFactor ?? 1.2);

    if (hMax < hMin) [hMin, hMax] = [hMax, hMin];

    const maxFenceH = Math.max(12, groundY - 20); // не выше «неба»
    ...
    // Возможен двойной — зазор растёт с прогрессом скорости
    if (Math.random() < (SPACING.doubleFenceChance ?? 0.4)) {
      ...
      const scale = sFrom + (sTo - sFrom) * prog; // 1.0 → 1.5 (по умолчанию)
    ...
    // Низкое облако — фиксированная нижняя грань хитбокса
